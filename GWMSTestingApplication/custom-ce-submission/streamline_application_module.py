...
def submit(jobenvfile):
    # Call condor_submit
    p = subprocess.Popen(["/usr/kerberos/bin/condor_submit", jobcon_direct], env=jobenvfile, stdout=subprocess.PIPE, stderr=subprocess.STDOUT) #subprocess. tells python where the name is coming from
    p.wait()
    ...
    with open(jobenv_direct) as new:
        ...
        for i in lines:
            try:
                ...
                pass
    jobenvfile["GLIDEIN_COUNT"] = str(args.GLIDEIN_COUNT)
